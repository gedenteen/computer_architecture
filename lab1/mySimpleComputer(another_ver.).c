...
//extern ram[], registr; //указывает, что эти переменные объявлены в другом файле
//extern OVERFLOW, DIVISON_BY_ZERO, GOING_BEYOND_MEMORY, IGNORING_CLOCK_PULSES, WRONG_COMMAND;
...
int sc_memorySave(char *filename)
{
	...
	{ //Если же файл не может быть открыт, возвращается NULL.
		fclose(fp);
		printf("Ошибка: не удалость открыть файл %s \n", filename);
		return 1;
	}
	if ( !fwrite(ram, sizeof(int), 100, fp) )
